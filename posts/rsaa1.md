[Or Cohen](/index.html)
# Regime Based Strategic Asset Allocation - Part 1
...
A common joke around statisticians is that if your feet are in the oven and your head is in the freezer, on average, your body is fine. When it comes to portfolio management though, variation of returns is as important as the average. This is the core principal that underlies the Mean-Variance asset analysis that was introduced by Harry Markowitz in 1952 and was since adopted by the vast majority of portfolio managers.
...
Shu et al. introduce in their research [1] a hybrid framework that on a high level, augments asset allocation methods such as mean-variance optimization (MVO) with asset-specific return and risk forecasts based on a binary regime prediction (Bull or Bear) to determine the optimal asset allocation weights.
...
def algorithm1(X1, X2, returns, jump_penalty, pred_start, pred_end):
	...
	for date in biannual_windows:
		lookback_window = slice(date - pd.DateOffest(years=11, date))
		prediction_window = slice(date, date + pd.DateOffset(months=6, date))
		jm = JumpModel(n_components=2, jump_penalty=jump_penalty, cont=True)
		# fit jump model
		jm.fit(X1.loc[lookback_window], returns.loc[lookback_window], sort_by="cumret")
		# make labels
		reqime_labels = jm.predict(X1)
		# fit xgboost
		bst = XGBClassifier(objective='binary:logistic')
		predictors = pd.concat([X1,X2], axis=1)
		bst.fit(predictors.loc[lookback_window], reqime_labels)
		# predict the next half year
		preds = bst.predict(predictors.loc[prediction_window])
...
In addition, this framework creates a synergy between the models used in the both stages and provides us with a tailored, financially-optimized regime forecasting model for each asset class. 
...
In the next post we will see how we can incorporate the regime forecasts in our portfolio management and discover the benefits of doing so.
